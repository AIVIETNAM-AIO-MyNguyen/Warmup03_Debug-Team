# 1. ToxiGuard AI and the Toxic Comment Detection Problem in Online Learning Communities

As classrooms gradually move into online environments, comment sections are no longer just spaces for asking questions or exchanging knowledge. They have become an essential part of the digital learning experience, directly reflecting the communication culture within learning communities. However, alongside the rapid growth of online learning platforms comes the increasing presence of offensive, abusive, and hateful comments. These types of content not only reduce the quality of discussions but also negatively impact users’ mental well-being and motivation to learn.

In online learning forums, just a few toxic comments can make the discussion environment feel tense and unsafe. Learners may hesitate to ask questions, avoid participating in discussions, or even leave the community altogether because they feel attacked or ridiculed. For platforms with large user bases, manually moderating every comment is nearly impossible due to the amount of time and resources required. This creates a growing demand for systems that can support the fast and automated detection of harmful content.

To address this issue, the team developed ToxiGuard AI as a support system for moderating toxic comments in online learning environments. The goal of the system is not to completely replace human moderators, but rather to serve as an additional support layer that helps identify potentially harmful content at an early stage. This allows administrators to respond more quickly and effectively while maintaining a positive learning environment for users.

To build such a system, the core challenge that must be solved is Toxic Comment Detection — the task of detecting harmful comments using artificial intelligence. Toxic comments can be understood as comments containing insults, harassment, threats, hate speech, or other forms of harmful behavior in online environments. This problem belongs to the field of Natural Language Processing (NLP), where computers are trained to understand and analyze human language. More specifically, toxic comment detection is commonly treated as a text classification problem, in which an AI model reads a comment and predicts whether it contains toxic elements or not.

However, toxicity does not always appear in a single form. In many modern datasets, a comment can be assigned multiple labels depending on the type and severity of the attack. Among these labels, “toxic” is often considered a general category for harmful or negative content. “Insult” refers to personal attacks, “threat” is related to intimidating or fear-inducing language, while “obscene” describes comments containing vulgar or offensive expressions. In addition, a single comment may belong to several categories at the same time. For example, a sentence containing both insults and profanity may be labeled as both insult and obscene.

For instance, the sentence “You are stupid” can be considered a direct insult. Meanwhile, sarcastic or indirectly offensive comments such as “Wow, what an intelligent answer…” are much harder to detect because their actual meaning depends heavily on context and tone. This is why toxic comment detection is not simply about filtering banned words, but instead requires machine learning models capable of understanding the meaning and context of natural language.

As online education continues to grow rapidly, applying AI to content moderation is not only a technological advancement but also a way to build a safer, more positive, and more respectful learning environment. ToxiGuard AI was developed with this goal in mind: supporting learning communities in maintaining healthy communication without disrupting users’ learning and discussion experience.

# 2. Why Is Toxic Comment Detection Difficult?

Although toxic comment detection is one of the most common problems in NLP today, building a system that can accurately identify toxic comments in real-world situations is far from simple. The challenge comes from the complexity of natural language itself and the way people communicate online.

The first difficulty lies in the ambiguity of language. A sentence can carry completely different meanings depending on the context in which it is used. For example, some expressions that appear offensive may simply be harmless jokes between friends, while seemingly polite comments can sometimes contain sarcasm or hidden insults. This makes it difficult for AI systems to accurately determine whether a comment is toxic based only on the surface meaning of the text.

In addition, many harmful comments are not expressed through direct attacks but instead through sarcasm, passive-aggressive language, or emotional manipulation. Humans can often recognize these subtle tones thanks to social experience and contextual understanding. However, for AI models, this remains a major challenge. Without sufficient training, a system may fail to detect dangerous comments or incorrectly label harmless comments as toxic.

Another issue is data imbalance. In practice, normal comments significantly outnumber toxic ones. As a result, models tend to learn the “majority pattern” and prioritize predicting comments as non-toxic in order to achieve high accuracy. However, an effective moderation system cannot rely on accuracy alone, because the more important factor is the ability to correctly identify genuinely harmful content.

Furthermore, toxic comment detection is also a multi-label classification problem. A single comment may belong to several categories at the same time. For example, a sentence containing both insults and vulgar language may be labeled as both insult and obscene. This requires the model to learn how to recognize multiple forms of toxicity simultaneously instead of simply classifying comments as either “toxic” or “non-toxic.”

Internet language also changes constantly. Users frequently use abbreviations, intentional misspellings, or special characters to bypass moderation systems. For example, offensive words may appear in forms such as “i.d.i.o.t,” “1d10t,” or even be replaced with emojis and memes. Without regular updates, a moderation system’s detection performance can quickly decline over time.
For these reasons, toxic comment detection is not just a technical challenge but also a problem closely connected to linguistics, user behavior, and social communication contexts. A good AI moderation model is not one that simply removes every negative comment, but one that helps humans make moderation decisions more accurately and fairly.
...
- **Insult:** Targets acts of insult, defamation, or undermining the reputation of a specific individual in the discussion.
...
Figure 4.1. Proportion of harmful and wholesome comments
</p>
...
<em>Figure 4.2. Frequency of occurrence of 6 harmful labels</em>
</p>
...
<em>Figure 4.3. Distribution of comment length (Minimum 400 words)</em>
</p>
...
Figure 4.4. Correlation matrix diagram between 6 toxic labels.
</p>
...
Figure 4.5. Frequency distribution chart of noise information
</p>
